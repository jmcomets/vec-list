...
template<class T>
struct Sort<vec_list<T> > {
    inline static void run(vec_list<T> &c, std::size_t){
        c.sort();
    }
};
...
struct RandomSortedInsert {
    ...
    inline static void run(Container &c, std::size_t size){
        for(std::size_t i=0; i<size; ++i){
            ...
            // hand written comparison to eliminate temporary object creation
            c.insert(std::find_if(std::begin(c), std::end(c), [&](decltype(*std::begin(c)) v){ return v.a >= val; }), {val});
        }
    }
};
